 ...
 */
	function sort($title, $key = null, $options = array()) {
		...
		$url = array_merge(array('sort' => $key, 'direction' => $dir, 'page' => 1), $url, array('order' => null));
	 ...
	 * ### Options:
	 *
	 * - `limit` each page's number set
	 * - `model` The model to use, defaults to PaginatorHelper::defaultModel()
	 */
	function limit($title, $options = array()) {
		$options = array_merge(array('url' => array(), 'model' => null), $options);
		$url = $options['url'];
		unset($options['url']);
		...
		$url = array_merge(array('limit' => $options['limit']), $url, array('order' => null));
		return $this->link($title, $url, $options);
	}
	 ...
	 * - `model` The model to use, defaults to PaginatorHelper::defaultModel()
	 */
	function statues($title, $options = array()) {
		$options = array_merge(array('url' => array(), 'model' => null), $options);
		$url = $options['url'];
		unset($options['url']);
		...
		$url = array_merge(array('status' => $options['status']), $url, array('order' => null));
		return $this->link($title, $url, $options);
	}
	 ...
	 * ### Options:
	 *
	 * - `contribution` each page's number set
	 * - `model` The model to use, defaults to PaginatorHelper::defaultModel()
	 */
	function contribution($title, $options = array()) {
		$options = array_merge(array('url' => array(), 'model' => null), $options);
		$url = $options['url'];
		unset($options['url']);
		...
		$url = array_merge(array('contribution' => $options['contribution']), $url, array('order' => null));
		return $this->link($title, $url, $options);
	}
 ...
 */
	function link($title, $url = array(), $options = array()) {
		...
		$html = $this->{$obj}->link($title, $url, $options);
		
		if(isset($this->params['url']['OSSN'])){
			$pram = "?OSSN=".$this->params['url']['OSSN'];
			if(isset($this->params['url']['ecsmall'])){
				$pram .= "&ecsmall=".$this->params['url']['ecsmall'];
			}
			$html = preg_replace('/href="([^"]+)"/',"href=\"$1$pram\"",$html);
		}
	...
	function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$out .= $this->Html->tag($tag, $i, array('class' => 'current num'));
				} else {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options),array('class' => 'num'));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
 ...
 * @access public
 */
	function show_limit_numbers($options = array()) {
		if ($options === true) {
			$options = array(
				'before' => ' | ', 'after' => ' | ', 'first' => 'first', 'last' => 'last'
			);
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			$out .= $before;
			/** e-agency liupeihua 2010-07-08 */
			$limit = $params['options']['limit'];
			$count = $params['count'];
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					$from = ($i -1)*$limit + 1;
					$to = ( $i*$limit > $count ) ? $count : $i*$limit;
					$out .= $this->Html->tag($tag, $from."位"."～".$to."位", array('class' => 'current'));
				} else {
					$from = ($i -1)*$limit + 1;
					$to = ( $i*$limit > $count ) ? $count : $i*$limit;
					$out .= $this->Html->tag($tag, $this->link( $from."位"."～".$to."位", array('page' => $i), $options));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}

	...
	function radio($fieldName, $options = array(), $attributes = array()) {
		...
		foreach ($options as $optValue => $optTitle) {
			...
			if( !empty( $attributes['li_class'] ) ) {
				$out[] = sprintf(
					$this->Html->tags['li'], " " , sprintf(
						$this->Html->tags['radio'], $attributes['name'],
						$tagName, $parsedOptions, $optTitle
					)
				);							
			}else if( !empty( $attributes['div_class'] ) ) {
				$out[] = sprintf(
					$this->Html->tags['block'], " class='colL nowrap' " , sprintf(
						$this->Html->tags['radio'], $attributes['name'],
						$tagName, $parsedOptions, $optTitle
					)
				);							
 ...
 * @link http://book.cakephp.org/view/1429/radio
 */
	function radio_reverse($fieldName, $options = array(), $attributes = array()) {
		$attributes = $this->_initInputField($fieldName, $attributes);
		$legend = false;
		...
		foreach ($options as $optValue => $optTitle) {
			...
			if( !empty( $attributes['li_class'] ) ) {
				$out[] = sprintf(
					$this->Html->tags['li'], " " , sprintf(
						$this->Html->tags['radio_reverse'], $optTitle, $attributes['name'],
						$tagName, $parsedOptions
					)
				);							
			}else if( !empty( $attributes['div_class'] ) ) {
				$out[] = sprintf(
 ...
 */
	function select($fieldName, $options = array(), $selected = null, $attributes = array()) {
		...
		$li_class = null;
		$div_class = null;
		if (isset($attributes) && array_key_exists('multiple', $attributes) && $attributes['multiple'] === 'checkbox') {
			$showEmpty = '';
		} else {
			$showEmpty = '選択してください';
		}	
		...
		/** liupeihua */
		if (!empty($attributes['li_class'])) {
			$li_class = $attributes['li_class'];
			unset($attributes['li_class']);
		}
		if (!empty($attributes['div_class'])) {
			$div_class = $attributes['div_class'];
			unset($attributes['div_class']);
		}
		...
		$select = array_merge($select, $this->__selectOptions(
			...
			array(),
			$showParents,
			array('escape' => $escapeOptions, 'style' => $style, 'li_class' => $li_class, 'div_class' => $div_class)
		));
 ...
 */
	function year($fieldName, $minYear = null, $maxYear = null, $selected = null, $attributes = array()) {		
		if( empty( $minYear ) )$minYear = date( "Y", strtotime( "-100 years" ) );
		...
		/** e-agency liupeihua 2010-07-08 #start */
//		if( empty( $selected ) ) $selected = date( "Y" );
		/** e-agency liupeihua 2010-07-08 #start */
